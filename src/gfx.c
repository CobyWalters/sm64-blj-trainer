...
color_t gfx_blend_colors(color_t color_1, int color_1_weight, color_t color_2, int color_2_weight) {
    color_t color;
    color.r = (color_1.r * color_1_weight + color_2.r * color_2_weight) / (color_1_weight + color_2_weight);
    color.g = (color_1.g * color_1_weight + color_2.g * color_2_weight) / (color_1_weight + color_2_weight);
    color.b = (color_1.b * color_1_weight + color_2.b * color_2_weight) / (color_1_weight + color_2_weight);
    color.a = (color_1.a * color_1_weight + color_2.a * color_2_weight) / (color_1_weight + color_2_weight);
    return color;
}

bool gfx_display_lock(display_context_t disp) {
    /* Try to grab a render buffer */
    if (!(disp = display_lock())) return true;
    gfx->disp = disp;
    return false;
}
